...
{
    public partial class EqualButton : UserControl
    {
        private string text;
        ...
        private void EqualButton_Paint(object sender, PaintEventArgs e)
        {
            Graphics g = e.Graphics;
            g.SmoothingMode = SmoothingMode.AntiAlias;
            
            Color begin_color = Color.FromArgb(255, 150, 100);
            Color end_color = Color.FromArgb(240, 140, 65);
            if (state == BtState.Enter)
            {
                begin_color = Color.FromArgb(150, 255, 150, 100);
                end_color = Color.FromArgb(150, 240, 140, 65);
            }
            Rectangle rect = new Rectangle(e.ClipRectangle.X, e.ClipRectangle.Y, e.ClipRectangle.Width - 4 <= 0 ? 1 : e.ClipRectangle.Width - 4, e.ClipRectangle.Height - 4 <= 0 ? 1 : e.ClipRectangle.Height - 4);
            int radius = 15;
            //绘制阴影
